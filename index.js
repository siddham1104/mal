...
// const storage = multer.diskStorage({
//   destination: function (req, file, cb) {
//     // change the location of your downloaded folder here 
//     cb(null, 'D:/Downloads/apk-inspector/apk-inspector-0.1.0');
//   },
//   filename: function (req, file, cb) {
//     cb(null, file.originalname);
...
//   try {
//     const filePath = req.file.path;
//     const permissions = await main(filePath); // Get permissions from the main function
//     console.log(permissions);
//     const result = await predictMalicious(permissions); // Predict maliciousness using Python
//     res.json({ message: 'Analysis completed. Malicious prediction: ' + result });
//   } catch (error) {
//     console.error(chalk.red('Error:', error.message));
//     res.status(500).json({ error: 'An error occurred during analysis.' });
//   }
...
// async function predictMalicious(permissions) {
//   console.log(permissions);
  
//   return new Promise((resolve, reject) => {
//     PythonShell.run('D:/Downloads/apk-inspector/apk-inspector-0.1.0/predict.py', { args: JSON.stringify(permissions) }, (err, results) => {
//       if (err) {
//         reject(err);
//       } else {
//         resolve(results[0]);
...
async function predictMalicious(permissions) {
  return new Promise((resolve, reject) => {
    const pythonProcess = spawn('python', ['predict.py', JSON.stringify(permissions)]);
    ...
    pythonProcess.stderr.on('data', (data) => {
      console.error(chalk.red('Error in predictMalicious (stderr):', data.toString()));
      reject(data.toString());
    });

    pythonProcess.on('close', (code) => {
      if (code !== 0) {
        console.error(chalk.red(`Python process exited with code ${code}`));
